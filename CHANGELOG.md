...
- **Free Canvas Board & Cookie Persistence**: Added a dedicated "Free Canvas Board" view (`canvas` mode) allowing users and external/guest visitors to freely position, drag, resize, and configure widgets. Canvas layouts are persisted client-side in browser cookies (`er_canvas_layout_v1`) to offer a private, unauthenticated desktop experience.
